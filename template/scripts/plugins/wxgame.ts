...
export class WxgamePlugin implements plugins.Command {
    ...
    async onFinish(pluginContext: plugins.CommandContext) {
        //拷贝template文件
        const files = fs.readdirSync(path.join(pluginContext.projectRoot,'template/wxgame'));
        for(let i = 0; i < files.length; i++) {
            const fileName = files[i];
            const filePath = path.join(pluginContext.projectRoot, 'template/wxgame/' + fileName)
            const outPath = path.join(pluginContext.outputDir, fileName)
